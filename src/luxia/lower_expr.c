...
{
    const limba_typeinfo *x = ti(L, t);
    if (!limba_types_is_discrete(&L->S->ts, t) || !(x->flags & LIMBA_TF_RANGE))
        return false;
    const limba_typeinfo *b = ti(L, lxs_base(L->S, t));
    return x->lo > b->lo || x->hi < b->hi;
}
...
static void invalidate(lxl *L, limba_id p, uint64_t off, limba_ltype t)
{
    ...
    if (x->kind == LIMBA_LTK_ARRAY) {
        /* the first element, then copies that double what is done */
        const limba_typeinfo *ix = ti(L, x->index);
        uint64_t esize = ti(L, x->elem)->size;
        ...
        while (done < total) {
            ...
            uint32_t o[3] = {
                ...
                lxl_iconst(L, LIMBA_T_I64, (int64_t)chunk)};
